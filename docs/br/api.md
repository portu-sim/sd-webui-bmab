...
Quando usar a API do Stable diffusion webui para gerar imagens, você pode fazer uma chamada de API usando BMAB conforme mostrado abaixo.
...
Os argumentos do BAMB são os mesmos do arquivo de configuração salvo, e todos os ajustes podem ser utilizados com base nisso. Caso não esteja configurado, será usado o valor padrão.

Abaixo estão os valores padrão da configuração em formato json.
