...
class StableDiffusionProcessingTxt2ImgOv(StableDiffusionProcessingTxt2Img):
    ...
    def sample(self, conditioning, unconditional_conditioning, seeds, subseeds, subseed_strength, prompts):
        with KohyaHiresFixPreprocessor(self):
            ...
            self.rng = rng.ImageRNG(shape=shape, seeds=seeds, subseeds=self.subseeds, subseed_strength=self.subseed_strength, seed_resize_from_h=self.seed_resize_from_h, seed_resize_from_w=self.seed_resize_from_w)
            x = self.rng.next()
